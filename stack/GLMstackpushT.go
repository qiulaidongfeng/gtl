...
func (s *GLMstack) TsPushint(x int) error {
	s.mutex.RLock()
	s.pushrecord()                                  //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Intsize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
...
func (s *GLMstack) TsPushint8(x int8) error {
	s.mutex.RLock()
	s.pushrecord()                                  //入栈记录
	_, safe, nsize := s.tspushsafetycheck(Int8size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushint16(x int16) error {
	s.mutex.RLock()
	s.pushrecord()                                    //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Int16size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushint32(x int32) error {
	s.mutex.RLock()
	s.pushrecord()                                    //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Int32size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushint64(x int64) error {
	s.mutex.RLock()
	s.pushrecord()                                    //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Int64size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushuint(x uint) error {
	s.mutex.RLock()
	s.pushrecord()                                   //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uintsize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushuint8(x uint8) error {
	s.mutex.RLock()
	s.pushrecord()                                    //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uint8size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushuint16(x uint16) error {
	s.mutex.RLock()
	s.pushrecord()                                     //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uint16size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushuint32(x uint32) error {
	s.mutex.RLock()
	s.pushrecord()                                     //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uint32size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushuint64(x uint64) error {
	s.mutex.RLock()
	s.pushrecord()                                     //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uint64size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushByte(x byte) error {
	s.mutex.RLock()
	s.pushrecord()                                   //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Bytesize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushRune(x rune) error {
	s.mutex.RLock()
	s.pushrecord()                                   //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Runesize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushBool(x bool) error {
	s.mutex.RLock()
	s.pushrecord()                                   //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Boolsize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushFloat32(x float32) error {
	s.mutex.RLock()
	s.pushrecord()                                      //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Float32size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushFloat64(x float64) error {
	s.mutex.RLock()
	s.pushrecord()                                      //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Float64size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushUintptr(x uintptr) error {
	s.mutex.RLock()
	s.pushrecord()                                      //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Uintptrsize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushComplex128(x complex128) error {
	s.mutex.RLock()
	s.pushrecord()                                         //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Complex128size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushComplex64(x complex64) error {
	s.mutex.RLock()
	s.pushrecord()                                        //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Complex64size) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
...
func (s *GLMstack) TsPushInterface(x interface{}) error {
	s.mutex.RLock()
	s.pushrecord()                                        //入栈记录
	sl, safe, nsize := s.tspushsafetycheck(Interfacesize) //入栈安全检查
	if safe != safeOk {
		s.mutex.RUnlock()
		s.scap = s.tsaddcap(nsize)
		s.mutex.RLock()
